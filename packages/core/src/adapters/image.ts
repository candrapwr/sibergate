...
export async function image(call: AdapterCall): Promise<Response> {
  ...
  if (isKling) {
    ...
    return sendUpstream({ url, provider, body: JSON.stringify({ ...rest, model_name: model }), signal, passthroughHeaders: call.passthroughHeaders, dispatcher: call.dispatcher });
  }
  return sendUpstream({ url, provider, body: JSON.stringify({ ...body, model }), signal, passthroughHeaders: call.passthroughHeaders, dispatcher: call.dispatcher });
}
